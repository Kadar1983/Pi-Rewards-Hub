  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-purple-100 to-blue-100">
      ...
      <div className="p-6 text-center">
        ...
        <p className="mt-2">{logged ? "Logged in" : "Not logged in"}</p>
      </div>
      ...
      <div className="flex justify-around bg-white py-4 rounded-t-3xl shadow">
...
function Card({ title, onClick }) {
  return (
    <button
      onClick={onClick}
      className="p-5 rounded-2xl bg-gradient-to-r from-purple-500 to-blue-500 text-white font-bold shadow"
    >
      {title}
    </button>
  );
...
function Nav({ label, onClick }) {
  return (
    <button onClick={onClick} className="font-semibold text-gray-700">
      {label}
    </button>
  );
    }
